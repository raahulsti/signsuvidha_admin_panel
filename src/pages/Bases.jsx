...
export default function Bases() {
  ...
  const { data: basesResp, isLoading } = useGetBasesQuery({ page: 1, limit: 200 });
  ...
  const filteredData = useMemo(() => {
    ...
    return (Array.isArray(bases) ? bases : []).filter((row) => {
      const matchesSearch =
        !term ||
  ...
  }, [bases, productTypeFilter, searchText]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingRow) {
        ...
        await updateBase({ id: editingRow.id, body: fd }).unwrap();
        message.success('Base updated');
      } else {
        const fd = buildFormData(values, true);
        await createBase(fd).unwrap();
  ...
  return (
    <>
      ...
      >
        <h2 style={{ margin: 0 }}>Bases</h2>
        <Space wrap>
          <Input
            allowClear
            prefix={<SearchOutlined />}
            placeholder="Search base / product type"
            style={{ width: 260 }}
            value={searchText}
      ...
      <Modal
        title={editingRow ? 'Edit Base' : 'Create Base'}
        open={modalOpen}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            name="name"
            label="Base Name"
            rules={[{ required: true, message: 'Please enter base name' }]}
          >
            <Input placeholder="e.g. Acrylic base" />
          </Form.Item>
